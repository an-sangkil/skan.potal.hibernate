...
var personalCodeAuction = (function () {
	return {
		moveForm : function () {
			document.code_form.action = "${pageContext.request.contextPath}/personal_code/personal_code_form";
			document.code_form.submit();
		},
...
<form name="code_form" method="post" action="${pageContext.request.contextPath}/personal_code/personal_code_list">
...
<div class="row">
	<div class="col-md-4 col-md-offset-8" align="right">
		<!-- Contextual button for informational alert messages -->
		<button type="button" class="btn btn-info" onclick="personalCodeAuction.moveForm()">
	...
	<table class="table table-striped">
		...
		<tbody>
			<c:forEach var="item" items="${codePage.content}" varStatus="status">
				<tr onclick="personalCodeAuction.detailView('${item.code}')">
					...
					<td>
						${item.upperCode }
						<c:if test="${!empty item.upperPersonalCode }">
						(${item.upperPersonalCode.codeName})
						</c:if>
					</td>
					<td>
						<c:choose>
